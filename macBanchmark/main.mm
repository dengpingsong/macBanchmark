...
// 并行生成和处理多个数组的最大值
void GPU_process_multiple_arrays(int trials, size_t array_size, bool is_int) {
    std::vector<std::thread> threads;
    int num_threads;
    size_t len = sizeof(num_threads);
    if (sysctlbyname("hw.ncpu", &num_threads, &len, nullptr, 0) != 0) {
        std::cerr << "Failed to get number of CPUs" << std::endl;
    }
    // 启动多个线程来并行生成和处理数据
    for (int t = 0; t < num_threads; ++t) {
        threads.push_back(std::thread([=]() {
            gpu_run(trials/num_threads+(t==0?trials%num_threads:0), array_size,is_int);  // 调用 GPU 处理
        }));
    }
...
int main() {
    ...
    if (use_gpu) {
        ...
        GPU_process_multiple_arrays(trials, array_size, is_int);
    } else {
        if (is_int) {
